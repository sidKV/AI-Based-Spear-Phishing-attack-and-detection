...
class EmailGeneratorAgent(Agent):
    ...
    def process(self, target_data: Dict, difficulty: str = "moderate") -> Dict:
        ...
        try:
            ...
            # Add a phishing URL - use custom URL if provided, otherwise generate one
            if "custom_url" in target_data:
                email_components["phishing_url"] = target_data["custom_url"]
            else:
                email_components["phishing_url"] = self._generate_phishing_url(difficulty, target_data)
            
            # Replace [LINK] with the URL if present
            if "[LINK]" in email_components["body"]:
                email_components["body"] = email_components["body"].replace(
                    "[LINK]", 
                    email_components["phishing_url"]["display_url"]
                )
...
class ContentAnalysisAgent(Agent):
    ...
    def _extract_urls(self, text: str) -> List[Dict[str,str]]:
        """Extract URLs from text and return them in a structured format"""
        import re
        # Fixed URL pattern - removed problematic character range
        url_pattern = r'https?://(?:[-\w.]|(?:%[\da-fA-F]{2}))+[^\s]*|www\.(?:[-\w.]|(?:%[\da-fA-F]{2}))+[^\s]*'
        urls = re.findall(url_pattern, text)
        return [{"display_url": url, "actual_url": url} for url in urls]
    ...
    def process(self, email_content: Dict) -> Dict:
        ...
        for url in urls:
            url_result = self.url_analyzer.process(url)
        ...
        try:
            ...
            # Add URL analysis results to the response
            analysis_result["found_urls"] = urls
            analysis_result["url_analysis"] = url_analysis_results
            
            # Adjust overall phishing probability based on URL analysis
            if url_analysis_results:
                url_scores = [result.get('url_legitimacy_score', 0.5) for result in url_analysis_results]
                avg_url_score = sum(url_scores) / len(url_scores)
                # Combine content and URL analysis (70-30 weight)
                analysis_result['overall_phishing_probability'] = (
                    analysis_result['overall_phishing_probability'] * 0.7 +
                    (1 - avg_url_score) * 0.3  # Invert URL score as higher means more legitimate
                )
